...
import { CastClass, CastHubLike, CastLambda } from '../hub';
...
export class CastRefactor implements CastRefactorLike {
    ...
    private readonly _methodLambda = $repo.newMap<PropertyReflectionLike, Array<CastLambda>>(FQN, '_methodLambda');
    private readonly _methodClasses = $repo.newMap<PropertyReflectionLike, Array<CastClass>>(FQN, '_methodClasses');
    ...
    property(ins: DecoInstanceLike, opt: CastOpt): void {
        ...
        const clazz = this.hub.discover.find(opt.type, () =>
            $dev.opt({
                kind: 'basic',
    ...
    parameter(ins: DecoInstanceLike, opt: CastOpt): void {
        ...
        if (!this._methodClasses.has(ref.property)) {
            this._methodClasses.set(ref.property, []);
            ref.property.listParameters().forEach((_p, index) => {
                this._methodClasses.get(ref.property)[index] = undefined;
            });
        }
        this._methodClasses.get(ref.property)[ref.index] = clazz;
        ...
        if (!this._methodLambda.has(ref.property)) {
            const lambdaList = [];
            ref.property.listParameters().forEach((_p) => {
                lambdaList.push((v) => v);
            });
            this._methodLambda.set(ref.property, lambdaList);
        }
        this._methodLambda.get(ref.property)[ref.index] = clazz.cast;
        this.logger.deploy.$info(FQN, 100, {
    ...
    hasMethod(ref: PropertyReflectionLike): boolean {
        return this._methodLambda.has(ref);
    }

    runForMethod(ref: PropertyReflectionLike, values: Array<any>): Array<any> {
        const lambdaList = this._methodLambda.get(ref);
        if (!lambdaList) {
            return values;
        }
