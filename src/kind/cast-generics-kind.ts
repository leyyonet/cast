import {$dev, $is, $log, $name, $repo, Func} from '@leyyo/common';
import { ClassReflectionLike, Fqn, fqnHandler, PropertyReflectionLike, reflectionPool } from '@leyyo/core';

import { FQN } from '../internal';
import { CastBase, CastClass, CastDocCallback, CastDocResponse, CastHubLike, CastName } from '../hub';
import { AssignGenericsOpt, CastBasic, CastGenerics, GenericsIndexOpt } from '../decorators';
import { CastGenericDefSign } from '../index.symbols';
import { CastGenericsKindLike } from './index.types';
import {CastTokenized} from "../process";

@Fqn(FQN)
export class CastGenericsKind implements CastGenericsKindLike {
    // region properties
    private logger = $log.create(CastGenericsKind);
    ...
    addIndex(fieldRef: PropertyReflectionLike, opt: GenericsIndexOpt): void {
        ...
        if (!classRef.hasDecorator(CastGenerics)) {
            throw $dev.developerError2(FQN, 100, {
                message: 'Class must be decorated by "CastDto" or "CastGenerics"',
                field: fieldRef.description,
            });
        }
        const bbb = this.hub.depot.get(classRef, true);
        bbb.value.push('from-generics');
        if (!bbb.value.generics) {
            bbb.value.generics = {
                produced: true,
                min: 0,
                max: opt.index + 1,
            ...
            const extGenerics = bbb.value.generics;
    ...
    processIndex(fieldRef: PropertyReflectionLike): void {
        ...
        if (def) {
            const clazz = this.hub.discover.find(def, () => $dev.opt({ where: 'CastGenericsKind', method: 'process' }));
    ...
    build(tokenized: CastTokenized): CastBase {
        ...
        const mainBase = this.hub.depot.get(tokenized.main);
        const extGenerics = mainBase.value.generics;
        const mainClass = mainBase.value.clazz;
        if (!mainBase.value.tags.includes('from-generics') || !extGenerics) {
            throw $dev.developerError2(FQN, 100, {
        ...
        const name = $name.anonymous(mainClass.name);
        $name.set(clazz, name);
